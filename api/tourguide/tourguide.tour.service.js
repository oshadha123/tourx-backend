...
module.exports = {
    addTour:(guideId,tourName,description,cost,days,nights,start,destination,vrUrl,path,attractionId,callBack)=>{
        pool.query(
          "INSER INTO tour(tourName,description,cost,days,nights,start,destination,vrUrl,guideId) VALUES (?,?,?,?,?,?,?,?,?);SET @lastInsertId = (LAST_INSERT_ID());INSERT INTO photo(path,tourGuideId,attractionId,tourId,userFlag,photoFlag) VALUES (?,?,?,@lastInsertId,0,0);SELECT @lastInsertId AS tourId;",
